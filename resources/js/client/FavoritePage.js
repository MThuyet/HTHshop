document.addEventListener('DOMContentLoaded', function() {
        ...
        products.forEach((product) => {
            ...
            productCard.innerHTML += `
                ...
                <div class="relative overflow-hidden cursor-pointer">
                        <img src="${assetStorageUrl}/${productImage}" alt="${product.name}"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                            <p class="text-orangeColor font-semibold text-[14px]">
                                ${(productPrice - (productPrice * productDiscount / 100)).toLocaleString('vi-VN')}đ
                            </p>
                            ${productDiscount > 0 ? `<p class="text-gray-400 line-through text-[12px]">${productPrice.toLocaleString('vi-VN')}đ</p>` : ''}
                        </div>
                    </div>
                </div>
            `;
    ...
    function removeProductWishList(productId) {
        ...
        .then(data => {
            if (data.status === true) {
                showToast('Xóa sản phẩm khỏi danh mục yêu thích thành công', 'success');
