document.addEventListener('DOMContentLoaded', function () {
    ...
    function addDropdownListener(container, dropdown, handler) {
        ...
        const newHandler = () => {
            container.querySelector('span.material-symbols-rounded').textContent = 'arrow_drop_up';
    ...
    function setupFilterInput(container, dropdown) {
        ...
        input.addEventListener('input', (event) => {
            ...
            dropdown.querySelectorAll('div > li').forEach((liElement) => {
                const matches = liElement.textContent.toLowerCase().includes(searchValue);
    ...
    function resetWard() {
        const wardText = '---';
        const newWardContainer = disableDropdown(wardContainer, wardDropdown, wardText);
    ...
    .then(data => {
        ...
        provinces.forEach((province) => {
            ...
            liElement.classList.add('px-[6px]', 'py-[6px]', 'hover:bg-[#66afe9]');
        ...
        districts.forEach((district) => {
            let liElement = document.createElement('li');
            liElement.classList.add('px-[6px]', 'py-[6px]', 'hover:bg-[#66afe9]', 'hidden');
        ...
        districtClickHandler = addDropdownListener(provinceContainer, provinceDropdown);
        ...
        document.addEventListener('click', (event) => {
            if (!provinceContainer.contains(event.target)) {
                provinceContainer.querySelector('span.material-symbols-rounded').textContent = 'arrow_drop_down';
                provinceDropdown.classList.add('hidden');
            }
            if (!districtContainer.contains(event.target)) {
                districtContainer.querySelector('span.material-symbols-rounded').textContent = 'arrow_drop_down';
                districtDropdown.classList.add('hidden');
            }
            if (!wardContainer.contains(event.target)) {
                wardContainer.querySelector('span.material-symbols-rounded').textContent = 'arrow_drop_down';
                wardDropdown.classList.add('hidden');
            }
        });

        provinceDropdown.querySelectorAll('div > li').forEach((liProvinceElement) => {
            liProvinceElement.addEventListener('click', (event) => {
                event.stopPropagation();
                const selectedValue = liProvinceElement.textContent;
                const previousValue = provinceContainer.querySelector('h5').textContent;
                
                provinceDropdown.querySelectorAll('div > li').forEach((e) => {
                    e.classList.remove('bg-[#eee]');
                });
                liProvinceElement.classList.add('bg-[#eee]');
                
                provinceContainer.querySelector('h5').textContent = selectedValue;
                provinceContainer.querySelector('ul > div')
                    .insertBefore(liProvinceElement, provinceDropdown.querySelector('div > li').nextSibling);
                ...
                if (selectedValue !== '---') {
                    const provinceCode = liProvinceElement.dataset.province_code;
                    inputHiddenProvince.value = selectedValue;

                    
                    districtContainer.classList.remove('bg-[#eee]');
                    
                    const newDistrictContainer = districtContainer.cloneNode(true);
                    districtContainer.parentNode.replaceChild(newDistrictContainer, districtContainer);
                    districtContainer = newDistrictContainer;
                    districtDropdown = districtContainer.querySelector('ul');
                    districtClickHandler = addDropdownListener(districtContainer, districtDropdown);
                    
                    setupFilterInput(districtContainer, districtDropdown);
                    
                    districtDropdown.querySelectorAll('div > li').forEach((liDistrictElement) => {
                        const isRelevant = liDistrictElement.dataset.province_code === provinceCode;
                        liDistrictElement.classList.toggle('hidden', !isRelevant);
                        
                        if (isRelevant) {
                            liDistrictElement.addEventListener('click', handleDistrictClick);
                        }
                    });
                } else {
                    const newDistrictContainer = disableDropdown(districtContainer, districtDropdown, districtText);
                    inputHiddenProvince.value = '';
                    districtContainer = newDistrictContainer;
                    districtDropdown = districtContainer.querySelector('ul');
                }
                
                districtContainer.querySelector('h5').textContent = districtText;
                wardContainer.querySelector('h5').textContent = wardText;
            });
        });
        ...
        function handleDistrictClick(event) {
            ...
            const previousValue = districtContainer.querySelector('h5').textContent;
            ...
            districtContainer.querySelector('h5').textContent = selectedValue;
            districtContainer.querySelector('ul > div')
                .insertBefore(liDistrictElement, districtDropdown.querySelector('div > li').nextSibling);

            if (selectedValue !== '---') {
                inputHiddenDistrict.value = selectedValue;
                const districtCode = liDistrictElement.dataset.district_code;
                
                wardContainer.classList.remove('bg-[#eee]');
                
                const newWardContainer = wardContainer.cloneNode(true);
                wardContainer.parentNode.replaceChild(newWardContainer, wardContainer);
                wardContainer = newWardContainer;
                wardDropdown = wardContainer.querySelector('ul');
                wardClickHandler = addDropdownListener(wardContainer, wardDropdown);
                
                setupFilterInput(wardContainer, wardDropdown);
                
                wardDropdown.querySelectorAll('div > li').forEach((liWardElement) => {
                    const isRelevant = liWardElement.dataset.district_code === districtCode;
                    liWardElement.classList.toggle('hidden', !isRelevant);
                    
                    if (isRelevant) {
                        liWardElement.addEventListener('click', handleWardClick);
                    }
                });
            } else {
                inputHiddenDistrict.value = '';
        ...
        function handleWardClick(event) {
            ...
            wardContainer.querySelector('h5').textContent = liWardElement.textContent;
            inputHiddenWard.value = liWardElement.textContent;
            
            wardContainer.querySelector('ul > div')
                .insertBefore(liWardElement, wardDropdown.querySelector('div > li').nextSibling);
        }
    })
    .catch(error => {
    ...
    const inputs = {
        ...
        specifyAddress: orderForm.querySelector('textarea[name="specifyAddress"]'),
