...
{{-- Header right side --}}
<div class="flex items-center gap-5">
    <button type="button" class="flex items-center p-2 rounded-lg border border-gray-400 hover:bg-[#0F6A9C] hover:text-white" id="search-btn-open-modal">
        <span class="material-symbols-rounded">
            search
        </span>
    </button>
    <button type="button" class="lg:hidden flex items-center p-2 rounded-lg border border-gray-400 hover:bg-[#0F6A9C] hover:text-white" id="menu-toggle">
        <span class="material-symbols-rounded">
            menu
        </span>
    </button>
    <div class="relative group inline-block">
        ...
        <ul class="absolute right-0 min-w-[150px] bg-white border shadow-sm rounded-md overflow-hidden z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition duration-150">
            <li>
...
{{-- Search modal --}}
<div id="search-modal" class="fixed inset-0 z-[9999] bg-black bg-opacity-50 flex items-start justify-center pt-24 px-4 hidden">
    <div id="search-dropdown" class="w-full max-w-xl bg-white rounded-lg shadow-lg p-4 relative">
        <div class="relative">
            <span class="material-symbols-rounded absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none">
                search
            </span>
            <input type="text"
                placeholder="Nhập chức năng bạn tìm kiếm..."
        ...
        <div id="search-dropdown-result" class="mt-3 max-h-60 overflow-y-auto border border-gray-200 rounded-md bg-white shadow-sm text-sm">
            <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 hover:bg-gray-100 text-gray-700">Quản lý sản phẩm</a>
        </div>
        <button class="close-modal-btn absolute right-[-10px] top-[-12px]" id="search-btn-close">
            <span class="material-symbols-rounded bg-white rounded-full h-[30px] w-[30px] leading-[30px!important] shadow-md">
                close
            </span>
        </button>
    </div>
